...
def create_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS command_queue
                 (id INTEGER PRIMARY KEY AUTOINCREMENT,
                  ...
                  command TEXT,
                  created_at TEXT,
                  sent INTEGER DEFAULT 0)''')  # Command queue table
    conn.commit()
    conn.close()
...
def update_dout1_state(imei, dout1_value, timestamp_str):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
def send_queued_commands(conn, imei):
    ...
    for cmd_id, command in commands:
        ...
        logging.info(f"Sent queued command for IMEI {imei}: {command}")
...
def parse_avl_packet(data, imei, conn):
    ...
    for _ in range(number_of_data):
        ...
        if dout1_value is not None and timestamp_str:
            command = update_dout1_state(imei, dout1_value, timestamp_str)
            if command:
                queue_command(imei, command)  # Queue automatic commands too
...
def control_dout1(imei):
    ...
    if row:
        ...
        if activate:
            deactivate_time = now + timedelta(seconds=ACTIVATION_DURATION)
            c.execute("UPDATE dout1_state SET dout1_active = 1, deactivate_time = ?, last_dout1_zero_time = NULL WHERE imei = ?",
                      (deactivate_time.strftime('%Y-%m-%d %H:%M:%S'), deactivatetime, imei))
            command = "setdigout 1"
        else:
            c.execute("UPDATE dout1_state SET dout1_active = 0, deactivate_time = NULL WHERE imei = ?", (imei,))
            command = "setdigout 0"
